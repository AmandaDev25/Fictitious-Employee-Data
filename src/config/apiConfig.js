...
import { codeErrorMessageHelper } from '../helpers/codeErrorMessageHelper';
...
const getResponseErrorMessage = (error, redirectLogin) => {
  if (error && error.message === 'Network Error') {
    return Promise.reject(codeErrorMessageHelper.NETWORK_ERROR);
  }

  if (error && error.message && error.message.includes('timeout of')) {
    return Promise.reject(codeErrorMessageHelper.TIMEOUT);
  }

  if (error && !error.response) {
    return Promise.reject(codeErrorMessageHelper.INTERNAL_SERVER_ERROR);
  }
